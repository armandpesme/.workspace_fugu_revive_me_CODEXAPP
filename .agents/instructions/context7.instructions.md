...
applyTo: '**/*.java,**/*.gradle,**/*.json,**/*.toml,**/*.md,**/*.mcmeta'
---
...
## How to use Context7 CLI (auto)

Context7 is available as a CLI tool (`ctx7`, installed globally via npm). Use it automatically as follows.

### Tool workflow

1) **If the user provides a library ID**, use it directly with `ctx7 docs`.
  - Valid forms: `/owner/repo` or `/owner/repo/version` (for pinned versions).

2) Otherwise, **resolve the library ID** using:
  - Command: `ctx7 library <name> [context] --json`
  - Example: `ctx7 library nextjs "app router" --json`

3) **Fetch relevant documentation** using:
  - Command: `ctx7 docs <libraryId> "your question"`
  - Example: `ctx7 docs /vercel/next.js "middleware configuration"`
  - Prefer `--json` for structured output when programmatic parsing is needed.

4) Only after docs are retrieved: **write the code/steps** based on those docs.

### Efficiency limits

- Do **not** call `ctx7 library` more than **3 times** per user question.
- Do **not** call `ctx7 docs` more than **3 times** per user question.
