...
name: screenshot
description: >-
  Utiliser quand l'utilisateur demande explicitement une capture d'écran bureau
  ou système (plein écran, application/fenêtre spécifique, ou région de pixels),
  ou quand les capacités de capture spécifiques à l'outil sont indisponibles et
  qu'une capture au niveau OS est nécessaire. Utile pour valider l'UI Minecraft
  (runClient), capturer des crashs visuels, ou documenter des rendus. Compatible
  GitHub Copilot, OpenCode, Anti-Gravity et Codex.
license: MIT
...
The helpers route Swift's module cache to `$TMPDIR/agent-swift-module-cache`
...
```bash
bash <path-to-skill>/scripts/ensure_macos_permissions.sh && \
python3 <path-to-skill>/scripts/take_screenshot.py --app "Antigravity"  # or your IDE/app name
```
...
```bash
python3 <path-to-skill>/scripts/take_screenshot.py --app "Antigravity"  # or your IDE/app name --window-name "Settings"
```

- List matching window ids before capturing (macOS only):

```bash
python3 <path-to-skill>/scripts/take_screenshot.py --list-windows --app "Antigravity"  # or your IDE/app name
```
