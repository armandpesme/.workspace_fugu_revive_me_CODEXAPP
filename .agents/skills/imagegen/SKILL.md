...
name: imagegen
description: >-
  Générer ou éditer des images raster quand la tâche bénéficie de visuels
  bitmap créés par IA : photos, illustrations, textures, sprites, mockups, ou
  cutouts à fond transparent. Utiliser quand l'agent doit créer une nouvelle
  image, transformer une image existante, ou dériver des variantes visuelles
  depuis des références, et que le résultat doit être un asset bitmap plutôt que
  du code ou du vectoriel. Utile pour générer des textures Minecraft, des
  mockups d'armures, ou des icônes de mod. Ne pas utiliser quand la tâche est
  mieux gérée par l'édition de SVG/vectoriel existant, l'extension d'un système
  d'icônes/logo, ou la construction visuelle directe en HTML/CSS/canvas.
license: MIT
...
- In built-in tool mode, the agent saves generated images under `$CODEX_HOME/*` by default.
