...
name: plugin-creator
description: >-
  Créer et scaffolder des répertoires de plugins pour Codex avec un
  `.codex-plugin/plugin.json` requis, des dossiers/fichiers optionnels, et des
  placeholders de base. Utiliser quand l'agent doit créer un plugin local,
  ajouter une structure optionnelle, ou générer/mettre à jour les entrées
  `.agents/plugins/marketplace.json` pour le tri et les métadonnées de
  disponibilité.
license: MIT
compatibility: codex
metadata:
