...
name: define-goal
description: >-
  Aider l'utilisateur à définir un objectif concret et mesurable avant de
  commencer le travail, surtout quand il demande à utiliser l'outil de goal,
  créer un goal, définir un objectif, clarifier les critères de succès, ou
  transformer une intention floue en résultat quantitatif. Utiliser pour la
  création et le raffinement d'objectifs uniquement ; ne gère pas les
  snapshots durables, logs de décisions, ou artefacts d'exécution longs.
  Complète le skill braintime pour le cadrage. Compatible GitHub Copilot,
  OpenCode, Anti-Gravity et Codex.
license: MIT
