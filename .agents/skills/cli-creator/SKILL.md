...
name: cli-creator
description: >-
  Créer un CLI composable pour l'agent depuis des docs d'API, une spec OpenAPI,
  des exemples curl existants, un SDK, une web app, un outil admin, ou un script
  local. Utiliser quand l'utilisateur veut créer un outil en ligne de commande
  qui peut s'exécuter depuis n'importe quel dépôt, exposer des commandes
  read/write composables, retourner du JSON stable, gérer l'auth, et s'associer
  à un skill compagnon. Utile pour créer des outils de modding Minecraft
  (extracteurs de textures, validateurs de recipes, etc.).
license: MIT
...
- Default to **Rust** for a durable CLI the agent should run from any repo: one fast binary, strong argument parsing, good JSON handling, easy copy/install into `~/.local/bin`.
